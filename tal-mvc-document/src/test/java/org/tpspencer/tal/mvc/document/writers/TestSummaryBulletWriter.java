...
public class TestSummaryBulletWriter {
	...
	public void basic() {
		...
		ChildWriters.SummaryBulletWriter writer = new ChildWriters.SummaryBulletWriter("test", "empty", "desc");
		
		context.checking(new Expectations() {{
			oneOf(element).getChildren("test"); will(returnValue(elems));
			oneOf(doc).startList();
			oneOf(doc).addListItem(with("desc"), with(element));
			oneOf(doc).endList();
		}});
	...
	public void emptyCheck() {
		...
		context.checking(new Expectations() {{
			oneOf(element).getChildren("test"); will(returnValue(null));
			oneOf(doc).writeParagraph("empty", null);
		}});
