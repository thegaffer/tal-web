...
public class TestInput {
	...
	public void basic() throws IOException {
		context.checking(new Expectations() {{
			oneOf(elem).getValue(model); will(returnValue("some Value"));
		}});
		
		Input inp = new Input(elem);
		inp.render(model);
		assertEquals("<input id=\"test\" value=\"some Value\" type=\"text\" name=\"test\"></input>", writer.toString());
